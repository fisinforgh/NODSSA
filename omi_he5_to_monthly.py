# scripts/omi_he5_to_monthly.py
# -*- coding: utf-8 -*-
"""
Convierte OMI Aura L3 (HE5, OMTO3e diarios) a CSVs mensuales y binned
que tu GUI 'χ² (Reales)' consume.

Salidas en --out_dir:
- bogota_monthly.csv                (Year,Month,Ozone_mean,Ozone_std,Count)
- bogota_binned.csv                 (sun_bin_center,y_mean,sigma)
- bogota_binned_minocc{M}.csv       (filtrando Count>=M, mismo formato binned)
- chi2_sweep.csv                    (min_occ,chi2_red_mean,pct_reduction)
"""
...
def _list_datasets(h: h5py.File):
    out = []
    h.visititems(lambda n, o: out.append(n) if isinstance(o, h5py.Dataset) else None)
    return out

def _guess_paths(h: h5py.File):
...
def _get_attr(attrs, *names, default=None):
...
def _apply_scale_offset(arr, attrs):
    scale = _get_attr(attrs, "ScaleFactor", "scale_factor", "Scale", default=1.0)
    offs  = _get_attr(attrs, "AddOffset", "add_offset", "Offset", default=0.0)
    try:
        return arr.astype("float64") * float(scale) + float(offs)
    except Exception:
        return arr.astype("float64")

def _reconstruct_latlon_from_grid(ds: h5py.Dataset):
...
def _read_he5(path: Path):
    with h5py.File(path, "r") as h:
        ...
        if o3_path is None: raise RuntimeError("No encontré ColumnAmountO3")
        ds = h[o3_path]
        o3 = np.array(ds[()])
        if o3.ndim == 3: o3 = o3[0, :, :]
        o3 = _apply_scale_offset(o3, ds.attrs)
        fill = _get_attr(ds.attrs, "_FillValue", "MissingValue", default=np.nan)
        if lat_path and lon_path:
            lat = np.array(h[lat_path][()]); lon = np.array(h[lon_path][()])
            if lat.ndim == 2 and lon.ndim == 2: lat, lon = lat[:,0], lon[0,:]
        else:
            lat, lon = _reconstruct_latlon_from_grid(ds)
...
def main():
    ...
    in_dir, out_dir = Path(a.in_dir), Path(a.out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    ...
    _to_binned_monthly(cut).to_csv(out_dir/f"bogota_binned_minocc{int(a.min_occ)}.csv", index=False)
