...
class Chi2BinsResult:
    N: int; nu: int; chi2: float; chi2_red: float
    p_two_sided: float; p_cdf: float; chi2_lo: float; chi2_hi: float
    a0: float; a1: float; sigma_a0: float; sigma_a1: float
    min_occ: Optional[int]; pct_reduction: float
    veredict_chi: bool; veredict_tutor: bool
...
def compute_chi2_from_bins(bins: pd.DataFrame, min_occ: Optional[int]=None, alpha: float=0.05):
    ...
    N = len(data)
    if N < 3: raise ValueError("Muy pocos bins tras el filtro (N<3).")
    ...
    xs = np.linspace(x.min(), x.max(), 200); ax.plot(xs, a0+a1*xs, lw=2,label="Ajuste ponderado")
    ax.set_xlabel("Manchas solares (bin)"); ax.set_ylabel("O₃ (DU)")
    ...
    ax.text(0.02,0.98,
        (f"N={N}, ν={nu}\nχ²={chi2:.3f}, χ²/ν={chi2_red:.3f}\n"
         ...
         f"PASA χ²: {'✅' if ver_chi else '❌'} / Tutor: {'✅' if ver_tutor else '❌'}\n"
    ...
    res = Chi2BinsResult(
        ...
        min_occ=min_occ, pct_reduction=pct, veredict_chi=ver_chi, veredict_tutor=ver_tutor
    )
    return res, fig
...
def sweep_min_occ(bins: pd.DataFrame, occ_values: Iterable[int]=range(3,21), alpha: float=0.05):
    ...
    for m in occ_values:
        try:
            ...
            rows.append({"min_occ":m,"chi2_red":r.chi2_red,"N":r.N,
                         "pct_reduction":r.pct_reduction,"p_two_sided":r.p_two_sided,
                         "p_cdf":r.p_cdf,"veredict_chi":r.veredict_chi,"veredict_tutor":r.veredict_tutor})
        except Exception:
            rows.append({"min_occ":m,"chi2_red":np.nan,"N":0,
    ...
    ax.set_xlabel("min_occ"); ax.set_ylabel("χ²/ν"); ax.grid(True, alpha=0.25); ax.legend(loc="best")
    return df, fig

# Entradas aceptadas para facilitar integración:
